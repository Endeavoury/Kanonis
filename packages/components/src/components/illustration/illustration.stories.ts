...
const meta: Meta = {
  title: 'Components/kanonis-illustration',
  tags: ['autodocs'],
  args: {
  ...
  argTypes: {
    ...
    variant: {
      control: 'select',
      options: ['empty', 'search', 'success', 'error'],
      description: "Public property ('empty' | 'search' | 'success' | 'error').",
    },
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Properties: `variant`, `label`. This component does not expose a slot. This component does not emit a custom event.',
      },
    },
  },
};
...
export const Usage: Story = {
  render: (args) => {
    ...
    const component = document.createElement('kanonis-illustration') as HTMLElement &
