...
- `kanonis-theme-toggle` for a persistent, accessible light/dark control;
- `kanonis-tabs` and `kanonis-tab` with automatic activation, roving tabindex, disabled tabs, and arrow/Home/End navigation;
- `kanonis-disclosure` using native details semantics;
- `kanonis-drop-zone` with file type/limit validation and typed accept/reject events;
- `kanonis-progress` for determinate and indeterminate progress;
- `kanonis-skeleton` with reduced-motion support.
...
A second breadth pass benchmarked the catalog against Material Design 3 and Bootstrap 5.3. It added form controls (`kanonis-textarea`, `kanonis-switch`, `kanonis-range`, and radio groups), native top-layer overlays, menus, tooltips, toast notifications, breadcrumbs, pagination, and flexible lists. See [Component breadth benchmark](component-parity.md) for the detailed capability matrix and deliberate exclusions.
...
| Theme behavior     | Tokens were complete; no reusable control existed             | Added `kanonis-theme-toggle` and application integration                                                     |
...
| Component coverage | P0 complete, important P1 workflows missing                   | Added six workflow primitives plus the `kanonis-tab` panel element                                           |
