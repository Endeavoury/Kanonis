...
its semantic tokens, Lit Web Components, Shadow DOM, `kanonis-*` events, selective registration, and thin
...
| Let `kanonis-grid`, `kanonis-stack`, `kanonis-inline`, and `kanonis-container` express a small reviewed set of responsive variants                         | Component  | P1       | Adapt; prefer semantic properties or container-query behavior over hundreds of utility classes                        |
...
| Specify compact, medium, and expanded behavior for feed, list-detail, and supporting-pane layouts                                                  | Pattern    | P1       | Adapt to `kanonis-pane-group`, `kanonis-pane`, and `kanonis-inspector-pane`                          |
...
| Harden `kanonis-data-table` and `kanonis-data-grid` for accessible names, row identity, repeated actions, busy/sort/page announcements, focusable overflow, numeric comparison, zoom, and text spacing | Component  | P0       | Adopt Primer's detailed acceptance model                           |
